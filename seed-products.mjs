...
async function seedProducts() {
  ...
  try {
    connection = await mysql.createConnection({
      ...
      user: process.env.DATABASE_URL?.split('//')[1]?.split(':')[0] || 'tienda_user',
      password: process.env.DATABASE_URL?.split(':')[2]?.split('@')[0] || 'tienda_password',
      database: process.env.DATABASE_URL?.split('/')[3] || 'mini_tienda',
    });
    ...
    for (const product of products) {
      try {
        await connection.query(
          'INSERT INTO products (userId, name, description, price, imageUrl, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, NOW(), NOW())',
          [userId, product.name, product.description, product.price, product.imageUrl]
        );
        insertedCount++;
    ...
    console.log(`\n✅ Se insertaron ${insertedCount}/${products.length} productos exitosamente`);
